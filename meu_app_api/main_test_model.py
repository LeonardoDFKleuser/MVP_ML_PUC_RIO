...
dataset_url = "database/steam.csv"  # Altere para o caminho correto do seu dataset Steam
...
# Verifica se as colunas categóricas estão presentes, caso contrário, insere valores padrões
...
# Transformar a coluna de preço em faixas categorizadas
Y = pd.cut(dataset[output_column], bins=[0, 5, 15, np.inf], labels=['Baixo', 'Médio', 'Alto']).astype(str).values  # Faixas de preço como alvo
...
print(f"F1 Score do modelo: {TestModel.get_f1_score():.3f}")
